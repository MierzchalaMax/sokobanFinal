...
class Link {
    ...
    set posX(_posX) {if (_posX){this.X = _posX;}}
    set posY(_posY) {if (_posY){this.Y = _posY;}}
    // ########## INSTANCE METHOD ##########
    // #####  Test de bord de map #####
    // depuis position de link
    hasTop() {return this.Y<=0 ? false : true;}
    hasBottom() {return this.Y>=6 ? false : true;}
    hasRight() {return this.X>=6 ? false : true;}
    hasLeft() {return this.X<=0 ? false : true;}
    // depuis position direction n+1
    hasNextTop(posY) {return (posY+1)<=0 ? false : true;}
    hasNextBottom(posY) {return (posY-1)>=6 ? false : true;}
    hasNextRight(posX) {return (posX+1)>=6 ? false : true;}
    hasNextLeft(posX) {return (posX-1)<=0 ? false : true;}
    // ##### Test de collision #####
    canTop() {
    ...
    moveBottom() {
        ...
        if (this.canBottom()) {
            ...
            if (Link.toTile(this.X, (this.Y+1)).classList.contains('ruby')) {
                Link.setEmpty(Link.toTile(this.X, (this.Y+1)));
                if (Link.toTile(this.X, (this.Y+2)).classList.contains('hole')) {Link.setRubyHole(Link.toTile(this.X, (this.Y+2)));}
                else {Link.setRuby(Link.toTile(this.X, (this.Y+2)));}
            }
            if (Link.toTile(this.X, (this.Y+1)).classList.contains('ruby_hole')) {
                Link.setHole(Link.toTile(this.X, (this.Y+1)));
                if (Link.toTile(this.X, (this.Y+2)).classList.contains('hole')) {Link.setRubyHole(Link.toTile(this.X, (this.Y+2)));
    ...
    moveRight() {
        ...
        if (this.canRight()) {
            ...
            if (Link.toTile((this.X+1), this.Y).classList.contains('ruby')) {
                ...
                if (Link.toTile((this.X+2), this.Y).classList.contains('hole')) {Link.setRubyHole(Link.toTile((this.X+2), this.Y));}
                else {Link.setRuby(Link.toTile((this.X+2), this.Y));}
            }
...
if (close) {close.addEventListener("click", eventManager);}
btnValid.addEventListener("click", eventManager);
    ...
    if (event.type == "keydown") {
        ...
        if(event.target.id==choixLvl){
            ...
            switch(choixLvl.selectedIndex){
                case 0: link = new Link(lvl1); break;
